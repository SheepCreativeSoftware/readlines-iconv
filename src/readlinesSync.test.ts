...
import { ReadLinesSync } from '../src/index.js';
...
describe('#readlinesSync', function () {
	it('Should return the same text line by line and should end with null', function () {
		...
		for (const currentLine of fileHandler) {
	...
	it('Should return done with true if closed', function () {
		const fileHandler = new ReadLinesSync({});
		fileHandler.open('./test/textfile.txt');
		fileHandler.close();
		assert.deepEqual(fileHandler.next().done, true);
		fileHandler.close();
	});
